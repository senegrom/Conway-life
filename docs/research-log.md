...
- **Exact BFS over reachable state sets** (`scripts/goe5_exact.py`, `modal/goe5_exact_modal.py`): dedup plus antichain pruning (supersets dropped — sound, since a subset empties no later). Height 4: 16 → 240 → 3,698 → 50,661 → 745,844 distinct sets at depths 1–5, i.e. **~15× growth per column, and the antichain filter removes only ~13%** (the sets are mostly incomparable). Certifies "no height-4 orphan of width ≤ 5" in 80 s and then exhausts memory; height 5 (32 letters) grows faster still.
