...
## 2026-09-02 — Reevaluation: f = 2 is a boundary-layer phenomenon; obstructions exist, minimal length 14

**Theory (elementary but decisive).** Write a pad¹ preimage in layers d0 (P's boundary layer), d1, d2, d3 outward. P's image depends on layers ≤ 1, ring R₁'s on d0,d1,d2, ring R₂'s on d1,d2,d3. Hence extending a pad¹ preimage to a pad² preimage is a purely one-dimensional problem along the boundary: keep (d0,d1), re-choose d2, choose d3 so that R₁ stays dead and R₂ dies. Define a boundary pair (d0,d1) as **level-2 deadly** if R₁ can be killed (pad¹-consistent) but no (d2,d3) kills both rings; **level-1 deadly** if no d2 kills R₁ (the f = 1 mechanism). f(P) = 2 ⟺ pad¹(P) SAT and *every* pad¹ preimage carries a level-2 deadly boundary. The same lemma applies at layers 2→3 and 3→4, so "no level-2 deadly boundary" would have implied pad¹ SAT ⇒ pad⁴ SAT ⇒ (Salo–Törmä) not GoE, i.e. optimal thickness exactly 1. The interior of P is irrelevant except as a forcing device — blind enumeration of P was the wrong tool.

**Computation.** `scripts/boundary_deadly.py` (brute force), `boundary_deadly_fast.py` (transfer matrices, numpy), `boundary_deadly_dfa.py` (subset construction, all lengths), `ring_deadly_sat.py` (complete rings with corners, SAT):

- Straight free-end segments: level-1 deadly words appear at L = 7 and grow ~5×/cell (4, 50, 384, 2368, 13040, 66913 at L = 7…12) — the f = 1 mechanism is abundant. **Level-2 deadly: none through L = 12** (16.7M pairs at L = 12), then the all-lengths DFA (2,651 reachable states) finds **121 deadly states; minimal deadly words have length 14: 32 of them**, then 202 / 1,052 / 4,967 at lengths 15/16/17 (`data/discoveries/deadly-boundary-words.txt`). Three examples independently re-verified by the DP and by a direct pysat encoding (R₁-sat, jointly UNSAT). Example: d0 = `00111000100000`, d1 = `00000000000111`.
- Complete rings (corners + cyclic closure): exhaustive 1×1, 1×2, 2×2, 2×3 and sampled 3×3 (3M), 4×4 (2M), 5×5 (1M): level-2 deadly rings **0**; level-1 deadly rings appear at 5×5 (966 in the sample), as expected from the straight L = 7 threshold.

**Consequences.**
1. **f = 2 witnesses are structurally possible** — the "constant is exactly 1" hypothesis is not supported by structure; the question is whether some pattern *forces* a deadly boundary on all its pad¹ preimages.
2. **Every f = 2 search so far was below threshold.** A straight-side obstruction needs a side of length ≥ 14; our censuses (≤ 13×13) and all flip sweeps could not contain one. This fully explains 3M+ clean negatives. The running D4 13×13 census cannot find f = 2 (barring an unexcluded corner mechanism at lengths 6–13); it remains a valid f = 1 census.
3. **Targeted route.** The two deadly features sit ~8 cells apart (a `111` run in d1 that must die, forcing a fence in d2; a `111`+`1` group in d0 constraining d2 from the other side; the d3 constraints then collide). Next: search for *forcing strips* — top strips S of width ≥ 14 all of whose preimages carry a deadly word — via the dual QBF ∀T₀ [boundary = W] ∃(T,d2′,d3): img T = img T₀ ∧ rings dead (36 universal bits at height 2, comparable to the solved four-row instances), and by measuring boundary-word rigidity of the existing f = 1 witnesses.

### 2026-09-02 (cont.) — rigidity measured; QBF route closed; ring-extension search launched

- **Rigidity is real.** The pop-69 witness has only **439 distinct top-boundary (d0,d1) words over all its bare preimages** (complete projected enumeration, 1 s); d0 takes essentially four values, and all 439 have live d1 cells — the alternating top row forces a live fence, i.e. the level-1 deadly mechanism. Patterns can pin their preimage boundaries hard.
- **Forcing-strip QBF parked.** `scripts/f2_strip_qbf.py` encodes ∀T₀[boundary = W] ∃(T,d2′,d3): img T = img T₀ ∧ rings dead (dual form). CAQE cannot decide even the trivial all-dead-boundary sanity instance (36 universal bits) in 5 min — the wall seen at four-row width 6. Deadly words have density ~1e-7 among 14-words, so forcing needs near-unique boundaries; a strip search would need height ≥ 5 (≥ 84 universal bits). Parked.
- **Ring-extension search (running).** `scripts/f2_ring_extend.py`: every D4 13×13 ringed orphan (~32k) and f = 1 witness (849) from the deep census, extended by each of the 256 D4-symmetric rings to 15×15 — rigid cores at exactly the threshold size, with the forced fence pushed one layer out. Smoke test: 14% of extensions relax pad¹ to SAT (the family sits on the boundary), ~20 candidates/s (19×19 windows). f = 1 cores first (217k candidates, 2 workers on cores 0–1), then the orphan cores (~8.2M, ~2.5 days). The 13×13 deep census — which cannot contain f = 2 — is paused with exact resume points (queue entries `w?br`) to free its two cores; it resumes on request as an f = 1 census.

### 2026-09-02 (later) — ring-extension families closed on Modal; forced boundaries never level-2 deadly

- **Modal fan-out works** (`modal/f2_ring_extend_modal.py`, Modal profile on this PC, image = debian-slim + python-sat, scripts mounted): 300 one-core containers, results identical to local runs. The D4 15×15 census (2³⁶) remains out of CPU reach (~$36k); a GPU pre-filter would be needed for that.
- **Orphan cores are useless as cores**: any extension of a bare orphan is a bare orphan (pad¹ never SAT) — the 8.3M-candidate f0 run confirmed pad¹-SAT = 0 instantly and was stopped. Only witness cores (bare SAT, pad¹ UNSAT) can seed f = 2.
- **One-ring D4 extensions of the 849 D4 13×13 witnesses** (217,344 candidates, 19% pad¹-SAT): f2 = 0. Unanchored asymmetric rings are hopeless as a family: one core alone admits > 150,000 pad¹-SAT rings (projected SAT, 870/s).
- **Deadly-word anchoring** (`f2_ring_anchored.py`): only a few 13×13 cores admit any deadly 15-word as a top boundary of a one-ring extension (cores 19: 18 words, 33: 2, 38: 36 among the first 43); deep enumeration (3,000 rings/word) of core 19: 54,000 rings, f2 = 0. For **two-ring extensions of 11×11 witnesses** (`f2_two_ring_anchored.py`) every deadly word is feasible for every core and rotation (5,280/5,280): two free rings supply any obstruction word — feasibility is not the bottleneck, forcing is.
- **D4 two-ring family complete on Modal**: 1,140 witnesses × 32,768 ring pairs = **37,355,520 candidates, 766,029 pad¹-SAT, f2 = 0** (87 s wall).
- **Forced boundaries are never level-2 deadly (so far).** Classifying every top-boundary word of eight witnesses' bare preimages by segment status: pop-84: 8 L1-deadly / 6 extendable; pop-73: 6 / 26; pop-69: 3 / 436; pop-69′: 2 / 198; four 13×13 witnesses: 8/194, 182/3270, 284/3660, 36/2218 — **zero L2-deadly among ~10,000 forced words**. Rigid patterns force fences that are either unkillable (L1) or freely killable; the "killable only in a doomed way" regime has not appeared naturally. Two-row anchored pruning for a census is too weak (88% of palindromic row pairs admit a deadly-word preimage).
- Next: build 15×15 witnesses (bare SAT, pad¹ UNSAT) from the two-ring family's pad¹-UNSAT members and run the single-flip boundary attack on them — the first boundary sweep above the length-14 threshold.

### 2026-09-02 (evening) — first boundary attack above the threshold: 15×15 witnesses, single flips, empty; GPU pre-filter ruled out

- **15×15 witness corpus** (`data/discoveries/witnesses15-two-ring.json`): classifying the whole two-ring D4 family on Modal (37.4M patterns): 36,583,799 bare orphans, **5,692 f = 1 witnesses** (bare SAT, pad¹ UNSAT) from 750 distinct 11×11 cores — the first witnesses of side ≥ 14.
- **Single-flip boundary attack on all 5,692** (Modal mode `flip15`, 300 containers, 710 s): **1,279,495 variants, 379,942 pad¹-SAT, 463,801 new single-flip 15×15 witnesses, f2 = 0, 0 mismatches.** Every witness perturbation that regained a pad¹ preimage also had a pad² preimage — the pad¹/pad² lockstep seen at 11 and 13 persists at the first size where level-2 deadly boundaries exist.
- **GPU pre-filter is not viable.** `scripts/preimage_anneal.py`: local search for pad² preimages of random D4 15×15 patterns finds 0/35 (random Metropolis, 20k steps × 3 restarts) and 4/39 (focused WalkSAT-style moves) of the SAT instances, at 0.7–3.9 s per candidate versus 0.36 s for the CDCL solver. The workload is CDCL-bound; a GPU cannot pre-filter it, and the 2³⁶ D4 15×15 census (~$36k of Modal CPU) stays out of reach.
- Policy from the owner: **all further searches run on Modal only; no local compute.** All local search processes stopped (anchored run, exhaustive ring tests, census remains paused with resume points).
- Standing state of the question: c\* ∈ {1,2,3,4}; every mechanism-informed family at the threshold size is empty; ~10k forced boundary words classified, none level-2 deadly. Next candidates (Modal): second-generation boundary attack on the 463,801 new witnesses (104M variants ≈ $800; a 10% sample ≈ $80), or the corner automaton to make "side ≥ 14" a theorem.

### 2026-09-02 (night) — second-generation boundary attack (10% sample) empty; Modal fat containers verified

- **Gen-2 attack** (`modal --mode gen210`, 60 fat containers × 8 worker processes, 18 min wall): the 5,692 first-generation 15×15 witnesses have **464,368 single-flip variants that are themselves f = 1 witnesses**; a deterministic 10% sample (46,800 of them, by raster hash) was attacked with all 225 single flips each: **10,530,000 variants, 3,116,704 pad¹-SAT, f2 = 0, 0 mismatches.** Full gen-2 identifier list saved in `build/modal-gen2-10-0-5692.json` (the other 90% can be attacked without recomputation).
- **Container layout verified** (`modal/cpu_probe_modal.py`): inside a `cpu=8` container, 8 CPU-bound worker processes run with 7.5× speedup (1 worker 2.0 s, 8 workers 2.2 s wall; even 16 workers 2.55 s — the host exposes 24 vCPUs and sets no cgroup cap, so containers can burst above the request). Throughput check: 10.5M near-boundary variants at ~10/s/core in 1,097 s implies ~900 concurrent cores on average; 8 cores would have needed ~36 h. The dashboard's per-function "CPU" trace shows a single container's usage, not the sum.
- Tally of the f = 2 hunt at the threshold size (all Modal, all empty): D4 one-ring (217k), D4 two-ring (37.4M), gen-1 flips (1.28M), gen-2 sampled flips (10.5M). Witness corpora: 5,692 gen-1 and 464,368 gen-2 fifteen-by-fifteen f = 1 witnesses.

### 2026-09-02 (late) — second-generation boundary attack complete at 100%: empty

- Remaining 90% (`--mode gen2rest`, hash residues 1–9; 120 fat containers, 62 min wall): 417,568 gen-2 witnesses attacked with all 225 single flips each — **93,952,800 variants, 27,826,148 pad¹-SAT, f2 = 0, 0 mismatches.** Exactly complementary to the 10% sample (46,800 + 417,568 = 464,368).
- **Combined 100%:** all 464,368 second-generation 15×15 witnesses, **104,482,800 boundary variants, 30,942,852 regained pad¹, every one of them also had pad².** With gen-1 (1.28M) and the ring families this brings the threshold-size tally to ~143M mechanism-informed candidates, all f2 = 0, plus ~3M below the threshold.
- Reading: at the first size where level-2 deadly boundaries exist, two generations of near-boundary perturbations of every known rigid pattern never produce a doomed-killable fence. The remaining moves are structural: the corner automaton (making "any f = 2 witness has side ≥ 14" a theorem) and the write-up to Salo–Törmä with the explicit f = 1 witnesses, the deadly-boundary characterisation, and the empty families.

## 2026-09-02 — new problems: the 45-cell record (C4 11×11, complete) and the narrowest five-row orphan

**Fork/PR housekeeping.** Both upstream PRs (binary-banter/fast-game-of-life#5, das67333/gol_engines#5) remain open with no maintainer response; our fix commits are merged into the forks' master/main and all inherited upstream WIP branches deleted (PR head branches kept so the PRs stay valid).

**C4-symmetric 11×11 orphans with ≤ 45 cells: complete census on Modal** (`scripts/c4_orphan_census.py`, `modal/c4_census_modal.py`; 150 fat containers, 17.7 min). Candidates = patterns with at most 11 of the 30 size-4 C4 orbits (centre free), i.e. every C4 11×11 pattern that could tie or beat the 45-cell record: **215,272,804 candidates, exactly one pad¹-orphan — the known 45-cell D4 pattern (population 45, verified).** Hence **no C4-symmetric 11×11 Garden of Eden with ≤ 44 cells exists, and the 45-cell one is the unique C4-symmetric 11×11 Garden of Eden with ≤ 45 cells** (`data/discoveries/c4-11x11-small-orphan-census.json`). The record stays at 45 for this symmetry class; beating it needs lower symmetry (2^61+ candidates) or larger boxes.

**Narrowest five-row orphan (Eker's 45×5 is the known one).**
- `scripts/goe5_beam.py`: column-transfer NFA (states = pairs of 7-bit preimage columns, 32 letters; orphan ⟺ reachable set empties), validated against the SAT checker (0/600 disagreements). Beam search on reachable-set size (beam 300, 16 seeds, widths ≤ 46): **no orphan** — sizes plateau at ~800–1,100 of 16,384 from width ~10 on.
- Eker's 45×5 (population 139, vertically mirror-symmetric) re-verified as an orphan; **none of its 1,034 proper contiguous sub-windows is an orphan** (it is sub-window-minimal). Its reachable-set trajectory stays at 700–3,800 states through column 33 and collapses only over the last twelve columns (428, 282, 98, 223, 356, 347, 177, 93, 157, 88, 2, 0): the collapse is structural, not greedy — set size is the wrong beam objective.
- Launched: exact depth-capped antichain search (BFS over reachable sets keeping inclusion-minimal ones; 96 GB container) — either the narrowest width or a rigorous lower bound on it.

### 2026-09-02 (late) — five-row orphan: anchored searches stall one column short of Eker; exact searches running

- Suffix-anchored prefix beams (fix Eker's last k columns, search a prefix whose reachable set avoids every state that can complete the suffix): k = 8/12/16/20, beam 1,500 and 3,000, 4 seeds each, with and without an exact two-column endgame lookahead — **no prefix shorter than Eker's** (45−k). The residue of completable states at the maximal shorter prefix is small but never zero: 48–60 for k = 20, 67–69 for k = 16 (beam 3,000), ~130–170 for k = 8–12 (beam 1,500). Eker's tail appears tight for its own mechanism.
- Running: exact antichain BFS over reachable sets (all 32 columns; and restricted to the 8 mirror-symmetric columns, which contain Eker's pattern and must therefore find an orphan by depth 45 — a built-in check). A completed mirror run decides whether 45 is the narrowest mirror-symmetric five-row orphan; the all-column run yields the exact minimum or a rigorous lower bound at the depth reached.
- Further beam spend held pending the exact results.

### 2026-09-02 (night) — five-row orphan: exact methods hit a wall; problem 2 closed as a negative

Three rigorous approaches were built and run on Modal; all certify only tiny widths, so the narrowest five-row orphan stays open with a huge gap (known upper bound 45, our rigorous lower bounds single-digit).

- **Exact BFS over reachable state sets** (`scripts/goe5_exact.py`, `modal/goe5_exact_modal.py`): dedup plus antichain pruning (supersets dropped — sound, since a subset empties no later). Height 4: 16 → 240 → 3,698 → 50,661 → 745,844 distinct sets at depths 1–5, i.e. **~15× growth per column, and the antichain filter removes only ~13%** (the sets are mostly incomparable). Certifies "no height-4 orphan of width ≤ 5" in 80 s and then exhausts memory. Height 5 measured: 32 → 992 → 31,582 → **1,009,097** distinct sets at depths 1–4 (489 s), all nonempty, i.e. **no five-row orphan of width ≤ 4** — and ~32× growth per column (no collapsing at all), so depth 5 needs ~32M sets and the method dies there.
- **Polynomial certificates** (`scripts/goe_lower_bound.py`): C_{n+1} = C_n ∩ ⋂_L step(C_n, L) makes R_w ⊇ C_{|w|} for every word, so C_n ≠ ∅ proves no orphan of width n. Result: collapses after 2 iterations at every height (**width ≥ 2**) — far too lossy, and notably it does **not** reproduce Wade's height-4 theorem, which would have required a nonempty fixed point.
- **k-lookback refinement** (sets indexed by the last k columns; converges to the exact BFS as k grows): k = 1 gives **no orphan of width ≤ 5 (height 4)** and **≤ 3 (height 5)** in seconds. Each extra lookback level buys ~2 columns at 32× the cost, so reaching width 45 would need k ≈ 15 (32¹⁵ indices). Dead end.
- Search side (already logged): suffix-anchored beams with exact endgame lookahead, beams to 3,000, never beat Eker's width; residues at one column short are 48–69 states.

**Verdict.** Our machinery cannot close the gap between 5-ish and 45; the narrowest five-row orphan needs a different idea (structural constructions, or an orders-of-magnitude better exact search). Problem 2 is closed as a negative with the artefacts kept. Problem 1 (the 45-cell record) delivered a real new fact: **no C4-symmetric 11×11 Garden of Eden has ≤ 44 cells, and the 45-cell one is unique among C4 11×11 GoEs with ≤ 45 cells.**
